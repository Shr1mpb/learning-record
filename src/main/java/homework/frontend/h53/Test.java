...
public class Test {
	...
	public Test() {
		 ...
		 * 3只要是两个汉字连起来就视为一个词。
		 * 
		 * 此题目不要求精确匹配答案，甚至不要求返回结果，自己输出结果和参考答案对比一下就行
		 * 注意：因为每个人程序实现稍微有所区别（例如对标点 空格 处理的不同 )，所以本文件给出的答案仅供参考 注意：
		 * 本题目批改的时候 只要可以编译通过并运行就可以，不评判具体返回字符等
		 */
		PoetryAnalysis util = new PoetryAnalysis();
		//chars 若干汉字组成的字符串以;分割，
		// 1统计这些汉字组成什么词汇出现在诗句里面，
		// 2按这些词汇出现的频率高低排序；
		// 3只要是两个汉字连起来就视为一个词
		String chars = "春;夏;秋;冬;暑;寒;风;雨;雪;霜;露";
		util.analysis("D:\\JavaProjects\\Common\\src\\main\\java\\homework\\frontend\\h53\\libai.txt", chars);
		util.analysis("D:\\JavaProjects\\Common\\src\\main\\java\\homework\\frontend\\h53\\dufu.txt", chars);
